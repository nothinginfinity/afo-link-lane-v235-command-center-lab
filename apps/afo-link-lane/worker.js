...
const VERSION = "2.3.19.1-node-chat-context-hardening";
...
function cvRenderEvidence(data){const status=document.getElementById('cvChatStatus');const target=document.getElementById('cvEvidence');const items=Array.isArray(data&&data.evidence)?data.evidence:[];const answerHtml=cvAnswerCard(data&&data.answer);if(!status||!target)return;if(!items.length){status.className='cvChatStatus empty';status.textContent='No evidence was found inside this node for that question.';target.innerHTML=answerHtml+'<div class="cvChatEmpty">Try a shorter question using words that appear in this resource.</div>';return;}status.className='cvChatStatus success';status.textContent=(data.answer&&data.answer.direct?'Direct extractive answer and ':'')+items.length+' evidence card'+(items.length===1?'':'s')+' retrieved only from '+String(data.resource_id||'this node')+'.';target.innerHTML=answerHtml+'<div class="cvEvidenceList">'+items.map(function(item,index){return cvEvidenceCard(item,data.source_sha256,index);}).join('')+'</div>';setTimeout(function(){target.scrollIntoView({block:'start',behavior:'smooth'});},40);}
