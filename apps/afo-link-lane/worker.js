...
function updateSearchFlight(){
  ...
  L.push(String.raw`async function openContentVisor(p){const visor=document.getElementById('contentVisor');if(!visor||!p)return;contentVisor.open=true;contentVisor.previousGameState=gameState;contentVisor.node=p;document.getElementById('focusBar').style.display='none';gameState='content_visor';visor.classList.add('open');visor.setAttribute('aria-hidden','false');const body=document.getElementById('cvBody');body.innerHTML='<div class="cvLoading">Loading in-app content engine...</div>';const yt=cvYoutubeId(p);if(yt){contentVisor.kind='youtube';cvSetShell(p,'YouTube Player');renderYoutubeVisor(p,yt);return;}const fileKind=cvFileKind(p);if(fileKind){contentVisor.kind='file';cvSetShell(p,fileKind);renderFileVisor(p,fileKind);return;}contentVisor.kind='reader';cvSetShell(p,'Article Reader');try{const r=await fetch('/content/read?url='+encodeURIComponent(p.url));const data=await r.json();renderArticleVisor(data,p);}catch(e){renderArticleVisor({ok:false,title:p.title,domain:p.domain,description:p.description,paragraphs:[]},p);}}`);
