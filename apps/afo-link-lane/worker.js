import { apiIndexPilotResources, apiQueryPilotResource, apiBrowserQueryPilotResource, apiPilotRetrievalStatus, VECTOR_INDEX_NAME } from "./resource-retrieval.js";

const VERSION = "2.3.18.15-node-local-retrieval-ui";
...
function updateSearchFlight(){
  ...
  L.push(String.raw`const CV_PILOT_RESOURCE_IDS=new Set(['fat-do-you-need-money-pdf','fat-money-management-checklist-pdf','fat-pslf-infographic-pdf','fat-how-financial-aid-works-graphic','fat-federal-student-loan-graphic']);
let cvRetrieval={sequence:0,controller:null,lastQuestion:'',resourceId:null};
...
function cvEvidenceCard(item,sourceSha,index){const source=String(item.source_url||'');const sourceLink=source?'<a class="cvEvidenceSource" href="'+cvEscape(source)+'" target="_blank" rel="noopener noreferrer">'+cvEscape(source)+'</a>':'Not available';const itemSourceSha=String(item.source_sha256||sourceSha||'');return '<article class="cvEvidenceCard"><div class="cvEvidenceHead"><span>Evidence '+(index+1)+'</span><strong>Score '+cvScore(item.score)+'</strong></div><div class="cvCitation">'+cvEscape(item.citation||'Citation unavailable')+'</div><div class="cvEvidenceText">'+cvEscape(item.text||'')+'</div><div class="cvEvidenceMeta">'+cvMetaRow('Resource ID',String(item.resource_id||''))+cvMetaRow('Page start',cvPageValue(item.page_start))+cvMetaRow('Page end',cvPageValue(item.page_end))+cvMetaRow('Chunk index',String(Number.isFinite(Number(item.chunk_index))?Number(item.chunk_index):'Not available'))+cvMetaRow('Chunk SHA-256',String(item.chunk_sha256||''))+cvMetaRow('Source SHA-256',itemSourceSha)+cvMetaRow('R2 chunk key',String(item.chunk_key||''))+'<div class="cvEvidenceRow cvEvidenceSourceRow"><span>Source URL</span><strong>'+sourceLink+'</strong></div></div></article>';}
function cvRenderEvidence(data){const status=document.getElementById('cvChatStatus');const target=document.getElementById('cvEvidence');const items=Array.isArray(data&&data.evidence)?data.evidence:[];if(!status||!target)return;if(!items.length){status.className='cvChatStatus empty';status.textContent='No evidence was found inside this node for that question.';target.innerHTML='<div class="cvChatEmpty">Try a shorter question using words that appear in this resource.</div>';return;}status.className='cvChatStatus success';status.textContent=items.length+' evidence card'+(items.length===1?'':'s')+' retrieved only from '+String(data.resource_id||'this node')+'.';target.innerHTML='<div class="cvEvidenceList">'+items.map(function(item,index){return cvEvidenceCard(item,data.source_sha256,index);}).join('')+'</div>';setTimeout(function(){target.scrollIntoView({block:'start',behavior:'smooth'});},40);}
...
function cvRetryResource(){if(cvRetrieval.lastQuestion)cvSubmitResourceQuestion(null,cvRetrieval.lastQuestion);}
async function cvSubmitResourceQuestion(event,questionOverride){if(event)event.preventDefault();const p=contentVisor.node;const resourceId=String((p&&p.id)||'');const input=document.getElementById('cvQuestion');const question=String(questionOverride!==undefined?questionOverride:(input&&input.value)||'').trim();if(!contentVisor.open||!CV_PILOT_RESOURCE_IDS.has(resourceId)){cvRenderChatError('This node is not enabled for the Financial Aid retrieval pilot.');return false;}if(question.length<3||question.length>500){cvRenderChatError('Enter a question between 3 and 500 characters.');return false;}if(input)input.value=question;if(cvRetrieval.controller)cvRetrieval.controller.abort();const controller=new AbortController();const sequence=++cvRetrieval.sequence;cvRetrieval.controller=controller;cvRetrieval.lastQuestion=question;cvRetrieval.resourceId=resourceId;cvSetChatBusy(true);const status=document.getElementById('cvChatStatus');const target=document.getElementById('cvEvidence');if(status){status.className='cvChatStatus loading';status.textContent='Searching evidence inside this node only…';}if(target)target.innerHTML='';if(input)input.blur();try{const response=await fetch('/api/resource-retrieval/query',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},credentials:'same-origin',cache:'no-store',body:JSON.stringify({resource_id:resourceId,question:question}),signal:controller.signal});const data=await response.json().catch(function(){return {ok:false,error:'The retrieval route returned invalid JSON.'};});if(sequence!==cvRetrieval.sequence)return false;if(!contentVisor.open||!contentVisor.node||String(contentVisor.node.id||'')!==resourceId)return false;if(!response.ok||!data.ok)throw new Error(data.error||('Retrieval returned HTTP '+response.status));if(data.resource_id!==resourceId)throw new Error('The retrieval response did not match the opened node.');const evidence=Array.isArray(data.evidence)?data.evidence:[];if(evidence.some(function(item){return !item||item.resource_id!==resourceId;}))throw new Error('Cross-node evidence was rejected by the Content Visor.');cvRenderEvidence(data);}catch(error){if(error&&error.name==='AbortError')return false;if(sequence===cvRetrieval.sequence)cvRenderChatError(error&&error.message?error.message:String(error));}finally{if(sequence===cvRetrieval.sequence){cvRetrieval.controller=null;cvSetChatBusy(false);}}return false;}
function cvBindResourceChat(){const form=document.getElementById('cvChatForm');const input=document.getElementById('cvQuestion');if(form)form.addEventListener('submit',cvSubmitResourceQuestion);if(input)input.addEventListener('focus',function(){setTimeout(function(){input.scrollIntoView({block:'center',behavior:'smooth'});},260);});}
function renderFileVisorRetrieval(p,kind){const url=String((p&&p.url)||'');const safeUrl=cvEscape(url);const title=cvEscape((p&&p.title)||'Official resource');const desc=cvEscape((p&&p.description)||'Official Financial Aid Toolkit resource.');const group=cvEscape((p&&p.group_name)||'Financial Aid Toolkit');const resourceId=String((p&&p.id)||'');let preview='';if(kind==='PDF File'){preview='<div class="cvFrameWrap cvFileFrame"><iframe src="'+safeUrl+'" title="'+title+'"></iframe></div>';}else if(kind==='Image File'){preview='<img class="cvHero cvFileImage" src="'+safeUrl+'" alt="'+title+'">';}else{preview='<div class="cvFallback"><h2>'+cvEscape(kind)+'</h2><p>This official file is tracked as a Link Lane resource node. Use Open Original to view or download the source file.</p></div>';}const enabled=CV_PILOT_RESOURCE_IDS.has(resourceId);const chat=enabled?'<section class="cvNodeChat"><div class="cvChatKicker">NODE-LOCAL RETRIEVAL</div><h3>Ask this exact resource</h3><p>Evidence is retrieved only from this opened node. No sibling-node or universe-wide fallback is active.</p><form id="cvChatForm" class="cvChatForm"><label for="cvQuestion">Question about this resource</label><textarea id="cvQuestion" class="cvQuestion" rows="2" maxlength="500" enterkeyhint="search" autocapitalize="sentences" placeholder="Ask about this resource…"></textarea><button type="submit" id="cvAskButton" class="cvAskBtn">Search This Node</button></form><div id="cvChatStatus" class="cvChatStatus ready" role="status" aria-live="polite">Ready to retrieve evidence from '+cvEscape(resourceId)+' only.</div><div id="cvEvidence" class="cvEvidence" aria-live="polite"></div></section>':'<section class="cvNodeChat"><div class="cvChatKicker">NODE CHAT SURFACE</div><h3>Retrieval is not indexed for this file</h3><p>The browser-safe node-local pilot is limited to five approved Financial Aid Toolkit resources.</p></section>';cvResetRetrieval(resourceId);document.getElementById('cvBody').innerHTML='<div class="cvFileCard"><div class="cvFileMeta">'+cvEscape(kind)+' · '+group+'</div><h2>'+title+'</h2><p>'+desc+'</p>'+preview+chat+'</div>';if(enabled)cvBindResourceChat();}
`);
...
function buildGameHTML(layout){
  ...
  const parts=[
    ...
    ".cvChatStatus.error{color:#ffb5b5;border-color:rgba(255,80,80,.48);}",
    ...
    "@media(max-width:560px){.cvFileCard{padding:14px}.cvNodeChat{padding:14px}.cvEvidenceCard{padding:13px}.cvEvidenceRow{grid-template-columns:1fr;gap:2px}.cvEvidenceText{font-size:15px}.cvAskBtn,.cvRetryBtn{min-height:52px}}",
